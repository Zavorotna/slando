<x-app-layout>
    <x-slot name="slot">
        <div class="p-6">
            <a class="text-2xl inline-block w-sm mb-10 py-3 px-6 duration-300 ease-in border border-black rounded-sm hover:bg-white hover:text-[#000]" href="{{ route('user.products.create') }}">Створити товар</a>
            <table class="w-full border border-black border-spacing-0">
                <thead>
                    <tr class="border-black text-center">
                        <th class="p-2 border-black border">Назва</th>
                        <th class="p-2 border-black border">Опис</th>
                        <th class="p-2 border-black border">Ціна</th>
                        <th class="p-2 border-black border">Ціна зі знижкою</th>
                        <th class="p-2 border-black border">Наявність</th>
                        <th class="p-2 border-black border">Знижка %</th>
                        <th class="p-2 border-black border">Редагувати</th>
                        <th class="p-2 border-black border">Видалити</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($products as $p)
                        <tr class="border-black text-center">
                            ...
                            <td class="p-2 border-black border-r">
                                @if($p->deleted_at)
                            ...
                            <td class="p-2 border-black border-r">
                                @if($p->deleted_at)
                                    <form action="{{ route('user.products.restore', $p->id) }}" method="post">
                                        ...
                                        @method('patch')
                                        <button type="submit">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="green"><path d="M440-320h80v-166l64 62 56-56-160-160-160 160 56 56 64-62v166ZM280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520Zm-400 0v520-520Z"/></svg>
                                        </button>
                                    </form>
                                @else 
