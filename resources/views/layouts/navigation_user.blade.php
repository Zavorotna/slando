<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        @if(Auth::user()) 
            {{-- currency input --}}
                <div class="currency space-x-8 sm:-my-px sm:ms-10 sm:flex text-gray-500 gap-2">
                {{-- {{ session()->forget('rates') }} --}}
                    @foreach (session()->get('rates') as $rate)
                        <form class="space-x-8 sm:-my-px sm:ms-10 sm:flex text-gray-500 gap-2" action="{{ route('admin.rates.update') }}" method="post">
                            @csrf
                            @method('patch')
                            <label class="flex items-center w-32 gap-3">
                                {{ strtoupper($rate['currency']) }}
